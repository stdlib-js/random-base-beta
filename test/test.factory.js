...
tape( 'the function returns a seeded pseudorandom number generator (array seed)', function test( t ) {
	...
	seed = [ now(), now()+1, now()+2, now()+3, now()+4 ];

	beta1 = factory( 2.0, 2.0, {
		'seed': seed
	});
	beta2 = factory( 2.0, 2.0, {
		'seed': seed
	});
...
tape( 'attached to the returned function is the generator name', function test( t ) {
	var beta = factory();
	t.strictEqual( beta.NAME, 'beta', 'has property' );
	t.end();
});

tape( 'attached to the returned function is the underlying PRNG', function test( t ) {
	var beta = factory();
	t.strictEqual( typeof beta.PRNG, 'function', 'has property' );

	beta = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( beta.PRNG, minstd.normalized, 'has property' );
	t.end();
});
...
tape( 'attached to the returned function is the generator seed (integer seed)', function test( t ) {
	...
	t.strictEqual( isUint32Array( beta.seed ), true, 'has property' );
	t.strictEqual( beta.seed[ 0 ], 12345, 'equal to provided seed' );

	beta = factory({
		'seed': 12345,
		'prng': minstd.normalized
	});
	t.strictEqual( beta.seed, null, 'equal to `null`' );
	t.end();
});
...
tape( 'attached to the returned function is the generator seed length', function test( t ) {
	var beta = factory();
	t.strictEqual( typeof beta.seedLength, 'number', 'has property' );

	beta = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( beta.seedLength, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the return function is the generator state', function test( t ) {
	var beta = factory();
	t.strictEqual( isUint32Array( beta.state ), true, 'has property' );

	beta = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( beta.state, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned function is the generator state length', function test( t ) {
	var beta = factory();
	t.strictEqual( typeof beta.stateLength, 'number', 'has property' );

	beta = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( beta.stateLength, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned function is the generator state size', function test( t ) {
	var beta = factory();
	t.strictEqual( typeof beta.byteLength, 'number', 'has property' );

	beta = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( beta.byteLength, null, 'equal to `null`' );
	t.end();
});
...
tape( 'attached to the returned function is a method to serialize the generator as a JSON object', function test( t ) {
	...
	beta = factory();
	t.strictEqual( typeof beta.toJSON, 'function', 'has method' );

	o = beta.toJSON();
	t.strictEqual( o.type, 'PRNG', 'has property' );
	t.strictEqual( o.name, beta.NAME, 'has property' );
	...
	t.strictEqual( typeof beta.toJSON, 'function', 'has method' );
	t.strictEqual( beta.toJSON(), null, 'returns expected value' );
...
tape( 'the function returns a PRNG for generating random numbers from a beta distribution (when `alpha` is large and `beta = 1`)', opts, function test( t ) {
	var threshold;
	var count;
	var npass;
	var alpha;
...
tape( 'the function supports specifying the generator state', function test( t ) {
	...
	for ( i = 0; i < 100; i++ ) {
		t.strictEqual( beta(), arr[ i ], 'returns expected value. i: '+i+'.' );
	}
	t.end();
});
...
tape( 'the returned function supports setting the generator state (alpha,beta <= 1.0)', function test( t ) {
	var state;
	var beta;
	var arr;
	var i;

	beta = factory( 0.25, 0.75 );
...
tape( 'the returned function supports setting the generator state (alpha == beta > 1.5)', function test( t ) {
	var state;
	var beta;
	var arr;
	var i;

	beta = factory( 3.0, 3.0 );
